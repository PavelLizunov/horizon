...
Use a short, accurate artifact title of no more than 15 words without clickbait; for languages that do not normally separate words with spaces, use one comparably short phrase. Write the entire artifact, block titles included, in the requested digest language and never mix languages. Use short block titles translated into that language (Russian: `Контекст`, `Суть`, `Вывод`; English: `Background`, `Solution`, `Takeaway`; Chinese: `背景`, `方案`, `启示`). Attribute arguments, interpretations, and unverified claims to the author rather than presenting them as established fact. Never invent data or missing details.
