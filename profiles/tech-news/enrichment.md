...
- `fact_check`: Extract the 2-3 most load-bearing verifiable factual claims (numbers, release dates, causal attributions) from the supplied content and check each with `web_search` against independent sources. In 1-3 complete sentences, state which claims are corroborated, which remain unverified, and which are contradicted, naming the independent sources. Omit the block when the content carries no verifiable claims (pure opinion, humor, or announcements without testable facts).
- `deep_dive`: Only when the supplied content is rich (full transcript or long-form article): in 4-6 complete sentences, drill into the most valuable concrete details — mechanisms, steps, tools, versions, numbers, caveats — that go beyond what `summary` already states. Omit the block for short or thin items.
