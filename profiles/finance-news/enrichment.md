...
- `fact_check`: Optional. When the item contains specific figures or claims attributed to a single source, use `web_search` to look for independent confirmation. In 1-2 short sentences, state whether the key figure or claim is corroborated by other outlets, unverified, or contradicted, naming sources. Omit the block for routine market commentary with no testable claims.
