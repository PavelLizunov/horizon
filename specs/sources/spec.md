...
* **Modes**: `inline` (run inside pipeline) or `sidecar` (`horizon-video` writes the configured inbox path, `data/video-inbox.json` by default).
...
* **Telegram** (`sources.telegram`): Public web previews through `telegram.me`, `telegram.dog`, and `t.me` fallbacks with rate-limit retries. A post's first external URL is canonical when present; the generated `telegram.me` post deep link remains in metadata.
...
* **Twitter/X** (`sources.twitter`): Apify `scweet` actor mode or Playwright with browser-cookie files; network and anti-bot failures degrade without aborting the pipeline.
